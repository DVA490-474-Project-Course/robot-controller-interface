...
void receive_data(int file_descriptor) {
    ...
    while (true) {
        ...
        ret = select(file_descriptor + 1, &read_file_descriptors, nullptr, nullptr, &timeout);
        if (ret == -1) {
...
int main() {
    ...
    int uart_file_descriptor =robot_controller_interface::hardware_interface::setup_uart(uart_device);
    ...
    std::thread receiver_thread
        (robot_controller_interface::hardware_interface::receive_data, uart_file_descriptor);

    /* Main loop to periodically transmit data through UART */
    const char* message = "Hello STM32!\n";
    while (true) {
        robot_controller_interface::hardware_interface::transmit_data(uart_file_descriptor, message);
    ...
    close(uart_file_descriptor);  /* Close the UART file descriptor before exiting */
